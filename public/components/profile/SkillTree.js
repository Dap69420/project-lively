function SkillTree() {
  try {
    ...
    if (progress.catalogStatus === 'loading') {
      return (
        <div className="glass-panel p-4 sm:p-8 flex-1 min-w-0 overflow-hidden" data-name="skill-tree" data-file="components/profile/SkillTree.js">
    ...
    const nodes = courses.map((course) => {
      ...
      const status = courseState.completed ? 'completed' : course.id === progress.selectedCourse ? 'in-progress' : courseState.xp > 0 ? 'in-progress' : 'locked';
      return {
        id: course.id,
        subject: course.name,
        level: course.focus,
    ...
    return (
      <div className="glass-panel p-4 sm:p-8 flex-1 min-w-0 overflow-hidden" data-name="skill-tree" data-file="components/profile/SkillTree.js">
        ...
        {courses.length === 0 ? (
          ...
          <div className="relative pl-5 sm:pl-10 border-l-2 border-white/10 space-y-8 sm:space-y-12 min-w-0">
            {nodes.map((node, idx) => (
              <div key={node.id} className="relative group min-w-0">
                {/* Connector dot */}
                ...
                <div className={`p-4 sm:p-5 rounded-xl border transition-all duration-300 min-w-0 overflow-hidden
                  ...
                  <div className="flex items-start justify-between gap-3 mb-2 min-w-0">
                    <div className="flex items-start gap-3 min-w-0">
                      <div className={`shrink-0 p-2 rounded-lg bg-black/40 ${node.status === 'completed' ? 'text-neonViolet' : node.status === 'in-progress' ? 'text-blue-400' : 'text-gray-500'}`}>
                        <div className={node.icon}></div>
                      </div>
                      <div className="min-w-0">
                        <h4 className="font-bold text-base sm:text-lg leading-snug break-words">{node.subject}</h4>
                        <p className="text-xs sm:text-sm font-mono text-gray-400 break-words">{node.level}</p>
                      </div>
                    </div>
                    ...
                    {node.status === 'locked' ? (
                      ...
                      <div className="shrink-0 text-neonViolet font-mono text-[10px] sm:text-xs border border-neonViolet/30 px-2 py-1 rounded bg-neonViolet/10">MASTERED</div>
                    ) : (
                      <div className="shrink-0 text-blue-400 font-mono text-[10px] sm:text-xs border border-blue-400/30 px-2 py-1 rounded bg-blue-400/10">ACTIVE</div>
                    )}
                  </div>
